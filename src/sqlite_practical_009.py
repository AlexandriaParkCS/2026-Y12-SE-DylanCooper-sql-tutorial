...
SQLITE_DB = (Path(__file__).parent / "../runtime/db/starwars.db").resolve()
...
def search_by_species(species):
    ...
    try:
        with sqlite3.connect(SQLITE_DB) as conn:
            ...
            cursor = conn.cursor()
            # Multiple parameters
            cursor.execute("""SELECT name, species, height FROM characters WHERE species = ? AND height IS NOT NULL ORDER BY height DESC""", (species,))
            results = cursor.fetchall()
            print(f"\n=== {species} Characters ===")
            for char in results:
...
def add_character(name, species, homeworld_id, height=None):
    ...
    try:
        with sqlite3.connect(SQLITE_DB) as conn:
            cursor = conn.cursor()
            cursor.execute(""" INSERT INTO characters (name, species, homeworld_id, height) VALUES (?, ?, ?, ?)""", (name, species, homeworld_id, height))
